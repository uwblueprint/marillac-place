...
import { AchievedBadgeLevel, EarnedCustomBadge } from "../../../../types/models";
...
export default function NewAchievedBadgesWidget({ pid }: NewAchievedBadgesWidgetProps) {
  ...
  const [fetchNewEarnedCustomBadges, { data: earnedCustomBadgeData, loading: earnedCustomBadgeLoading, error: earnedCustomBadgeError}] = useMutation(FETCH_NEW_EARNED_CUSTOM_BADGES);
  const [fetchNewAchievedBadgeLevels, { data: achievedBadgeData, loading: achievedBadgeLoading, error: achievedBadgeError}] = useMutation(FETCH_NEW_ACHIEVED_BADGE_LEVELS);
  useEffect(() => {
  ...
  const newEarnedCustomBadges = earnedCustomBadgeData?.fetchNewEarnedCustomBadges || [];
  const newAchievedBadges = achievedBadgeData?.fetchNewAchievedBadgeLevels || [];
  if (newAchievedBadges.length + newEarnedCustomBadges.length === 0) return null;
  ...
  return (
    ...
    >
      <Flex w="100%" justifyContent="space-between" alignItems="center" mb="8px">
        <Text textStyle="mobile.b0">New Badges Achieved!</Text>
        <UnderlineButton
          label="Progress"
      ...
      {newEarnedCustomBadges.length > 0 && newAchievedBadges.length > 0 && <Divider orientation="horizontal" color='neutral.300' mt="8px" />}
