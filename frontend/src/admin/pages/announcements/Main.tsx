...
export default function AdminAnnouncementsPage() {
  ...
  if (participantData?.getCurrentParticipants) {
    for (const participant of participantData.getCurrentParticipants) {
      ...
      participantToRoomMap[participant.pid] = participant.room;
    }
  }
  ...
  const announcementData = filter
    ? announcementsByParticipantsResult.data?.getAnnouncementsSentToParticipants
    : allAnnouncementsResult.data?.getAnnouncementsFromToday ?? [];
      ...
      <VStack spacing={4} align="stretch" paddingBottom="20px">
        ...
        {announcementData.map((announcement: Announcement) => (
          <AnnouncementCard
            ...
            room={
              announcement.ReceivedAnnouncement?.length === 1
                ? `Room ${
                    participantToRoomMap[
                      announcement.ReceivedAnnouncement[0].pid
                    ]
                  }`
                : announcement.ReceivedAnnouncement?.length ===
                  ROOM_NUMBERS.length
                ? "All Rooms"
                : `Rooms ${announcement.ReceivedAnnouncement?.map(
                    (ua: any) => participantToRoomMap[ua.pid]
                  ).join(", ")}`
            }
