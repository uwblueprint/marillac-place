...
import { Button, Text, Flex, Select, FormLabel, Spinner } from "@chakra-ui/react";

import { useMutation, useQuery, useLazyQuery } from "@apollo/client";
import { GET_AVAILABLE_ROOMS, GET_PARTICIPANT_BY_ID } from "../../../gql/queries";
...
const AddParticipantCard = ({ isOpen, setIsOpen }: AddParticipantCardProps): React.ReactElement => {
	...
	const validate = async () => {
		...
		if (participantId) {
			...
			} else if (getParticipantByIdData && getParticipantByIdData.getParticipantById != null) {
	...
	const handleSubmit = async () => {
		...
		if (!errors.participantId && !errors.roomNumber && !errors.arrivalDate && !errors.password) {
			try {
				const room = parseInt(roomNumber, 10);
				await createParticipant({ variables: { participantId, roomNumber: room, arrival: arrivalDate, password } });
				setIsOpen(false);
    ...
    return (
        <ModalContainer title="Add Participant" isOpen={isOpen} setIsOpen={setIsOpen}>
				...
				{ 
					getAvailableRoomsLoading ? (
						...
						<FormSelectField 
							...
							value={roomNumber}
							options={getAvailableRoomsData.getAvailableRooms.map((room: number) => ({
								key: room,
								value: room, 
								display: `Room ${room}`
							}))}
