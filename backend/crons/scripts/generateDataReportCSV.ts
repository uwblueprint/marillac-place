...
export async function generateWeeklyReportCSV(): Promise<boolean> {
  try {
    ...
    await fs.writeFile(path.join(reportsDir, filename), csvContent);
    ...
    console.log(`Weekly CSV report saved to: ${reportsDir}/${filename}`);
    return true;
  } catch (error) {
...
export async function generateMonthlyReportCSV(): Promise<boolean> {
  try {
    const reportsDir = path.join(__dirname, "../../../reports");
    await fs.mkdir(reportsDir, { recursive: true });
    ...
    console.log(`Monthly CSV report saved to: ${reportsDir}/${filename}`);
    return true;
  } catch (error) {
